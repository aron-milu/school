...
export default function SchoolAdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <nav className="sticky top-[72px] z-20 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto max-w-7xl px-4 py-3 sm:px-6">
          <div className="flex flex-wrap gap-2">
            {TAB_IDS.map(tab => {
              const label = tab === 'overview' ? 'Overview' : tab === 'classes' ? 'Classes' : tab === 'students' ? 'Students' : tab === 'teachers' ? 'Teachers' : 'Fees';
              ...
              return (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-sm font-semibold transition ${active ? 'bg-brand text-white shadow-sm' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>
                  {label}
                </button>
              );
            })}
      ...
      <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
        <div className="rounded-[2rem] border border-slate-200 bg-slate-50 p-6 shadow-sm">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-semibold text-brand">Welcome back, {user?.full_name?.split(' ')[0] || 'Admin'}.</p>
              <p className="text-sm text-slate-600">Your school dashboard is up to date with the latest attendance, schedule, and workflow insights.</p>
            </div>
            <div className="flex flex-wrap gap-3">
              <div className="rounded-full bg-white px-4 py-2 text-sm text-slate-600 border border-slate-200">Active term: Term 2, 2026</div>
              <div className="rounded-full bg-white px-4 py-2 text-sm text-slate-600 border border-slate-200">School: Lubiri Secondary</div>
            </div>
          </div>
        </div>
      </div>
